...
e1 <- c(5.3,3.9,3.1,3.1,3.1); #there are three line per column, the mean obj size changes per line as follows: 30, 45, 60
...
#229.142
#420.257
plot_colors <- colors()[c(507)]

pdf(file="output/packet_based_dur.pdf", 
...
par(mar=c(3.2,3,0.5,.1)) #margins, c(bottom, left, top, right)
...
x_labels<-c("0.5","1","2","4","8")

#plot(x, y, axes=F, type="n", 
#		xlab="Happiness (1000 clients and 400req/s)",
#		ylab="Storage Usage (TB)", ylim=c(0,max_y),
#		axes=FALSE, ann = FALSE)
plot(x, axes=F, type="n", ylim=c(0,6),
		ann = FALSE)

mtext(side = 2, text = "Time/msg.(microseconds)", line = 2, cex=global_cex)#, cex=global_cex)#, cex=.5) #line modifies mtext placement
mtext(side = 1, text = "Chunk size (MB)", line = 2, cex=global_cex)#, cex=global_cex)#, cex=.5)
...
axis(2, las=1, at=(0:(6)))#,cex.axis=.8,lwd=global_lwd)
