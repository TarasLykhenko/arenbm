...
e1 <- rev(c(10,15,23.2,39.9,89.7)); #there are three line per column, the mean obj size changes per line as follows: 30, 45, 60
#type <- mytable[["type"]]
#memosy usage(in MB): 9.5,9.4,9,9.2,9.3
#computation time (microseconds): 0.4,0.4,0.4,0.4,0.4

#shift_factor<-0.025;
#shift_y<-round(max_storage*shift_factor);
#shift_x<-round(max_happiness*shift_factor);

#for more colors, 
#see 
#http://research.stowers-institute.org/efg/R/Color/Chart/
plot_colors <- colors()[c(552)]

pdf(file="output/simple_provide_size.pdf", 
...
axis(2, las=1, at=10*(0:(max(e1))))#,cex.axis=.8,lwd=global_lwd)

lines(x_at,e1, type="o", pch=25, lty=1, 
